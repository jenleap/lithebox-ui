# Task 009: UI Pattern Registry

## Feature
F015 - Component Metadata System

## Description
Implement the `UIPattern` registry and define four high-level UI patterns: `AnalyticsKPIRow`, `SettingsFormLayout`, `DashboardHeader`, and `DataTableToolbar`. Patterns describe composed UI intent at a higher level than individual components and are the primary interface for AI-driven page generation.

## Files
- `src/metadata/patternRegistry.ts` (create)

## Implementation Steps

1. Create `src/metadata/patternRegistry.ts`.

2. Import `UIPattern` from `./types`.

3. Implement the pattern registry and its utilities:
...
4. Define and register the **AnalyticsKPIRow** pattern:
   ```ts
   {
...
5. Define and register the **SettingsFormLayout** pattern:
   ```ts
   {
     ...
     usage: [
       ...
       "Group related fields inside named sections",
...
6. Define and register the **DashboardHeader** pattern:
   ```ts
   {
     ...
     usage: [
       ...
       "Limit action group to 1 primary + 2 secondary buttons maximum"
     ],
     constraints: [
...
7. Define and register the **DataTableToolbar** pattern:
   ```ts
   {
     ...
     usage: [
       "Position directly above the Table component with no spacing gap",
...
8. Each `registerPattern(pattern)` call is a module-level side effect.

## Constraints
- No `any` types
- No React imports
- Pattern `structure` field uses indented plain-text pseudo-code, not JSX
- Pattern `components` must list only registered component names
- `_patternRegistry` must not be exported directly

## Acceptance Criteria
- `registerPattern`, `getPattern`, `getAllPatterns`, `exportPatternsAsJSON` are all exported
- All 4 patterns registered: `AnalyticsKPIRow`, `SettingsFormLayout`, `DashboardHeader`, `DataTableToolbar`
- `getPattern("AnalyticsKPIRow")` returns the correct pattern object
- `exportPatternsAsJSON()` returns parseable JSON containing all 4 patterns
- TypeScript compiles with no errors

## Test Steps
1. Run `npm run build` — no TypeScript errors
2. Import the file, call `getPattern("DashboardHeader")` — verify `components` includes `"Heading"` and `"Button"`
3. Call `exportPatternsAsJSON()` — parse the result and verify 4 keys are present
4. Call `getAllPatterns()` — verify return type is `Readonly<...>`

## Notes
Patterns are the highest-value output of F015 for AI consumption. Each pattern must be self-contained enough for an AI to generate a full layout from the `structure` field alone.
